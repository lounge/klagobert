...
class Messages {
  ...
  constructor(db, sentiment) {
    this.giphy = new Giphy('lPOTyab0KsIIWTVrgp7c2fqpZujzNPNx');
    this.imgur = new Imgur('8b8df9084d07948');
    this.cleverbot2 = new cleverbot();
    ...
    this.cleverbot2.configure({botapi: 'e87b82b674df063ad5aa4cd44a7f7935'});
  ...
  topWhines(callback) {
    ...
    this.db.getTopWhines(function(topWhines) {
      ...
      callback('', { title: 'Top whines: https://charts.mongodb.com/charts-klagobert-vgmxi/embed/charts?id=39d46ef1-058e-4ed2-872a-8b7ecea14d46&theme=dark', message: msg });
    });
  }
  ...
  scoreboard(callback) {
    ...
    this.db.getScoreboard(function(scoreboard) {
      ...
      callback('', { title: "Scoreboard: https://charts.mongodb.com/charts-klagobert-vgmxi/embed/charts?id=44c9dd43-e13c-4e99-9325-86cf49f2e95e&theme=dark", message: msg });
    });
  }
  ...
  getGiphy(msg, callback) {
    this.giphy.search({ q: msg }, function(err, data, res) {
      var length = data.data.length;
      var rand = Math.floor(Math.random() * length)
      var item = data.data[rand];
      callback(item.bitly_gif_url);
    });
  }

  kek(user, callback) {
    var word = this.sentiment.getRandomNegativeWord();
    this.giphy.search({ q: word }, function(err, data, res) {
  ...
  askCleverBot(callback, question) {
    ...
    this.cleverbot2.write(question, function (response) {
