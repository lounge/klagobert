  ...
  if (msgContent.indexOf('/') === -1) {
    ...
    if (command !== undefined) {
      ...
      if (cmd.match('(?:^|\s)(' + this.cmds.help.cmd + ')(?=\s|$)')) {
        ...
        this.wrongCommand(user.name, callback);
      }
    }
  }
};
...
Commands.prototype.kek = function(username, callback) {
  this.messages.kek(username, callback);
}

Commands.prototype.wrongCommand = function(username, callback) {
  this.messages.wrongCommand(username, callback);
}

Commands.prototype.askCleverBot = function(callback, question) {
