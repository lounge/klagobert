...
class Klagobert extends Bot {
  ...
  _onStart() {
    ...
    if (!this.started) {
      ...
      this.messages.scoreboard(function(msg) {
        self.sendMessage(msg);
      });
    }
    this.started = true;
  ...
  _calculateBitterPoints(score, msg, timestamp, username, negCallback, posCallback) {
    ...
    this.db.getBitterPointsForUser(username, function(currentBitterPoints) {
      ...
      if (score < 0) {
        var addon = Math.abs((Math.round((newVal + 0.00001) * 100) / 100));
        negCallback(username, msg, timestamp, currentBitterPoints, addon);
      } else {
        var newVal = currentBitterPoints - score;
        var bitterPoints = Math.abs((Math.round((newVal + 0.00001) * 100) / 100));
  ...
  _sendReaction(timestamp, streak) {
    ...
    this.postReactionToChannel(this.channels[this.channel].id, emoji, timestamp, {});
  };
  ...
  sendMessage(msg, data) {
    this.postMessageToChannel(this.channels[this.channel].name, msg, { as_user: true, unfurl_media: true });
  };
