...
Klagobert.prototype._analyzeMessage = function(msg) {
  ...
  if (this._isChatMessage(msg)) {
    ...
    console.log('user: ' + user + ' msg: ' + msgContent);
...
Klagobert.prototype._analyzeSentiment = function(msgContent, user) {
  ...
  this._calculateBitterPoints(result.comparative, user.name, msgContent, this._negativeScore.bind(this), this._positiveScore.bind(this));
};
...
Klagobert.prototype._calculateBitterPoints = function(score, username, msg, negCallback, posCallback) {
  this.db.getBitterPointsForUser(username, function(currentBitterPoints) {
    if (score < 0) {
      var roundedBp = currentBitterPoints + Math.abs((Math.round((score + 0.00001) * 100) / 100));
      negCallback(username, msg, roundedBp);
    } else {
      posCallback(username)
    }
  });
}

Klagobert.prototype._negativeScore = function(username, msg, bitterPoints) {
  this.db.updateScoreboard(username, bitterPoints);
  this.db.saveLastWhineForUser(username, msg);
  this._calculateWhineScore(msg);
  this._calculateWhineStreak(username);
}

Klagobert.prototype._positiveScore = function(username) {
  this.db.clearWhineStreak(username);
}

Klagobert.prototype._calculateWhineScore = function(whine) {
  var self = this;
  this.db.getWhineScore(whine, function(score) {
    self.db.saveWhineScore(msgContent, score + 1);
  });
}

Klagobert.prototype._calculateWhineStreak = function(username) {
  var self = this;
