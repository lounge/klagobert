  ...
  if (this._isChatMessage(msg)) {
    ...
    if (this._isChannelConversation(msg) && !this._isFromKlagobert(user.id)) {
        if (this._isMentioningKlagobert(msgContent)) {
          ...
          this._analyzeForSentiment(msgContent, user);
        }
    }
  }
};

Klagobert.prototype._analyzeForSentiment = function(msgContent, user) {
  var self = this;
  var result = this.sentiment.analyze(msgContent);
  this._calculateBitterPoints(result.comparative, user.name, function(bitterPoints) {
    self.db.updateScoreboard(user.name, bitterPoints);
    self.db.saveLastWhineForUser(user.name, msgContent);
    self._calculateWhineScore(msgContent, function(score) {
      self.db.saveWhineScore(msgContent, score);
    });
  });
};
...
Klagobert.prototype._calculateBitterPoints = function(score, user, callback) {
  this.db.getBitterPointsForUser(user, function(currentBitterPoints) {
    if (score < 0) {
      callback(currentBitterPoints + Math.abs((Math.round((score + 0.00001) * 100) / 100)));
    }
  });
}

Klagobert.prototype._calculateWhineScore = function(whine, callback) {
